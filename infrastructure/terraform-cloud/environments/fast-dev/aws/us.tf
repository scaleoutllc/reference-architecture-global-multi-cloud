...
resource "aws_iam_role" "tfc-fast-dev-aws-us" {
  name               = "tfc-fast-dev-aws-us"
 ...
 "Statement": [
   {
     ...
     "Condition": {
       ...
       "StringLike": {
         "${local.tfc.hostname}:sub": "organization:scaleout:project:${tfe_project.fast-dev-aws-us.name}:workspace:*:run_phase:*"
       }
     }
   }
 ]
...
resource "aws_iam_role_policy_attachment" "tfc-fast-dev-aws-us" {
  role       = aws_iam_role.tfc-fast-dev-aws-us.name
...
resource "tfe_variable_set" "fast-dev-aws-us" {
  name         = "fast-dev-aws-us"
  organization = "scaleout"
}

resource "tfe_variable" "fast-dev-aws-us" {
  for_each = {
    TFC_AWS_PROVIDER_AUTH = true
    TFC_AWS_RUN_ROLE_ARN  = aws_iam_role.tfc-fast-dev-aws-us.arn
  }
  variable_set_id = tfe_variable_set.fast-dev-aws-us.id
  key             = each.key
  value           = each.value
  category        = "env"
}
